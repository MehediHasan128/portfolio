...
const GlassIcons: React.FC<GlassIconsProps> = ({ items, className }) => {
  ...
  return (
    <div
      className={`grid gap-[5em] grid-cols-2 md:grid-cols-3 mx-auto overflow-visible ${className || ""}`}
    >
      {items.map((item, index) => (
        <button
          ...
          className={`relative bg-transparent outline-none border-none cursor-pointer w-[4.5em] h-[4.5em] [-webkit-tap-highlight-color:transparent] group ${
            item.customClass || ""
          }`}
        >
          <span
            className="absolute top-0 left-0 w-full h-full rounded-[1.25em] block transition-[opacity,transform] duration-300 ease-[cubic-bezier(0.83,0,0.17,1)] origin-bottom-right rotate-15 will-change-transform group-hover:transform-[rotate(25deg)_translate3d(-0.5em,-0.5em,0.5em)]"
            style={{
          ...
          >
            <span
              className="m-auto text-3xl flex items-center justify-center"
              aria-hidden="true"
            >
              {item.icon}
            </span>
